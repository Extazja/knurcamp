...
// Miejsce na Twój kod.
    ...
    function sprawdzam(a, b) {
        const klucza = Object.keys(a);
        // console.log(klucza);
        const kluczb = Object.keys(b);
        // console.log(kluczb);
        if (klucza.length !== kluczb.length) {
            return false;
        }
        for (const klucz in a) {
            if (a[klucz] !== b[klucz])
            return false;
        }
...
// Miejsce na Twój kod.
    ...
    function inzynieriaOdtylcowa(a) {
...
// Miejsce na Twój kod.
    ...
    function pascal(a) {
        // console.log(a);
        const klucze = Object.keys(a);
        // console.log(klucze[0]);
        return a[klucze[0]];
    }
...
// Miejsce na Twój kod.
    ...
    function xNzq(ksiazka) {
        // console.log(JSON.stringify(ksiazka));
        const poprawnaKsiazka = [];

        // function szukaj(s) {
        //     return s.strona == strona.strona
        // }

        for (const strona of ksiazka) {
            if (!poprawnaKsiazka.find((s) => s.strona == strona.strona)) {
                poprawnaKsiazka.push(strona);
            }
            
        }

        // console.log(JSON.stringify(poprawnaKsiazka));

        poprawnaKsiazka.sort((a,b) => a.strona - b.strona);

        // console.log(JSON.stringify(poprawnaKsiazka));

        return poprawnaKsiazka;
    }
...
// Miejsce na Twój kod.
    ...
    function detektor(ciag, wzor) {
        // console.log(ciag, wzor);
        let licznik = 0;
        let ilosc = 0;

        for (let i = 0; i < ciag.length; i++) {
            // console.log(ciag[i]);
            if (ciag[i] == wzor[licznik]) {
                licznik++;
                if (licznik == wzor.length) {
                    ilosc++;
                    licznik = 0;
                }
            }
...
// Miejsce na Twój kod.
    ...
    function mors(a) {
        ...
        alfabet = Object.fromEntries(Object.entries(alfabet).map(x => [x[1], x[0]]));
    
        const fragmenty = a.split(" ");
        // console.log(fragmenty);
        // console.log(alfabet);
        let slowo = "";

        for (const litera of fragmenty) {
            slowo += alfabet[litera];
            // console.log(litera);
            // console.log(alfabet[litera]);
            // console.log(slowo);
